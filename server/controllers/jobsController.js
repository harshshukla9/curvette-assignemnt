...
// ****************edit job *************************************
export const editJobController = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { status } = req.body;

    console.log('Edit job request received:', { jobId: id, status });

    // Find job
    const job = await jobsModel.findOne({ _id: id });
    console.log('Job found in database:', job ? 'Yes' : 'No');
    ...
    // Authorization check
    const userIdMatches = req.user._id.toString() === job.createdBy.toString();
    console.log('Authorization check:', { 
      requestUserId: req.user._id.toString(), 
      jobCreatorId: job.createdBy.toString(),
      authorized: userIdMatches 
    });

    if (!userIdMatches) {
      console.log('Authorization failed: User not authorized to edit this job');
      return next("You are not authorized to edit this job!");
    }
    ...
    if (status && !validStatuses.includes(status)) {
      ...
      return next("Invalid status value. Must be pending, interview, or rejected.");
    }

    console.log('Updating job with data:', { status });
    ...
    console.error('Error in editJobController:', error);
    next(error);
  }
};
